#'@title os.predict
#'
#'@description  Calculates Predict2.1 Overall survival and therapy benefits
#'
...
os.predict <- function(data,year=10,age.start,screen,size,grade,nodes,er,her2,ki67,generation,horm,
                       traz,bis){
  ...
  predict2.1s <- function(age.start,screen,size,grade,nodes,er,her2,ki67,generation,horm,
                          traz,bis){
    ##----------------------------------------------------------------
    #Fix inputs
    ...
    Overall <- round((1-pred.cum.all.rx[year,1])*100,2) #Overall
    horm.ben <- round(benefits2.1[year,2],2) #Hormonal therapy benefit
    chemo.ben <- round(benefits2.1[year,6]-benefits2.1[year,2],2) #Chemotherapy benefit
    traz.ben <- round(benefits2.1[year,12]-benefits2.1[year,6],2) #Trastuzumab benefit
    bis.ben <- round(benefits2.1[year,16]-benefits2.1[year,12],2) #Bisphosphonate benefit
  ...
  {N<-nrow(data) #Number of patients
    ...
    colnames(results)<-c(O.S,H.B,C.B,T.B,B.B)} #Rename columns

  for (n in 1:N) {
    results[n,]<- predict2.1s(data[[age.start]][n],data[[screen]][n],data[[size]][n],data[[grade]][n],data[[nodes]][n],
